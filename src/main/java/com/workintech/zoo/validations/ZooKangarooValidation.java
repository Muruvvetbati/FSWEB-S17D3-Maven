...
import org.springframework.http.HttpStatus;  // Spring HttpStatus sınıfı
...
public class ZooKangarooValidation {
    ...
    public static void checkKangarooWeight(Double weight) {
        if(weight == null || weight<= 0){
            throw new ZooException("weight shouldn't bu null or less then zero: "+ weight ,HttpStatus.BAD_REQUEST);
        }
    }
}